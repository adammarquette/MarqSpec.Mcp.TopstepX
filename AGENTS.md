...
- **Search with `git grep`, never an unscoped `grep -r`.** This checkout sits beside other sessions'
  ...
  `bin`/`obj` half alone, with no worktree needed: measured at `f0c723ef2d471861c8c1f30415cac98fa9be6b04` on
...
- **Before trusting a history count or a "root commit" claim, check whether the checkout is shallow.**
  `git rev-parse --is-shallow-repository`, then read `$(git rev-parse --git-common-dir)/shallow` for the graft
  points — a worktree has none of its own, it inherits the common dir's. Scoping the walk with `--remotes=origin`
  or `--branches` does not escape it: the graft sits below ref selection, so every ref stops at the same
  boundary. The failure is silent and plausible, not a timeout — a grafted commit reports as parentless and a
  truncated count reads as complete. This checkout **was** one, grafted at `08c96da`, until the maintainer ran
  `git fetch --unshallow` on 2026-09-06: at `4fb3832` the shallow walk counted **100** commits where the full
  one counts **460**, and `08c96da` showed no parent where it has `8677fba`. Measure history in an unshallowed
  clone — `--unshallow` on the shared checkout is the maintainer's call, since it rewrites shared object state
  under every concurrent session; a throwaway `git clone` outside the repo is anyone's. Same family as the
  `git grep` trap above; its remedy does not cover this one (gh#477, gh#485).
