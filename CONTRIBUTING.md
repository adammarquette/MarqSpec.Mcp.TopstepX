...
- **A citation inside code is not a citation** — a fenced block, a **four-space-indented** block, an HTML
  ...
  **Indenting a list continuation is not code, and you do not have to change how you write markdown.** The
  four columns are measured from the enclosing list item's content column and a blank line has to be above
  them, so a bullet's continuation, a nested item and a wrapped sentence all stay prose.
