    ...
    FootprintCacheService? footprints = null)
{
    ...
    [Description(
        ...
        + "with no recorded contract — history cached before this server tracked provenance — and that is "
        + "NOT a roll: read `contracts.span` to tell them apart. `SpansRoll` means a real roll; `Unknown` "
        + "means unattributed rows, a store defect rather than a market event, and every one of those bars "
        + "may still belong to the SAME contract the rest of the window does. Read `detectedOverBars` — "
