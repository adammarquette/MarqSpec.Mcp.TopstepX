    ...
    TimeProvider clock)
{
    ...
    [Description(
        ...
        + "zero even after a genuine fetch. Never returns a truncated series: a window over either cap — the "
    ...
    [Description(
        ...
        + "`venueRequests` and `fetchedBuckets`, and only the first is evidence of a vendor round trip: "
        + "`venueRequests` of 0 is the exact test for an answer served entirely from the store, while "
        + "`fetchedBuckets` counts the BASE buckets this call wrote or revised. Never returns a truncated "
        + "series: a count over this server's row cap, or one the calendar holds too few closed sessions to "
        + "satisfy, is refused naming the count. `contracts` says which contracts produced these sessions; "
