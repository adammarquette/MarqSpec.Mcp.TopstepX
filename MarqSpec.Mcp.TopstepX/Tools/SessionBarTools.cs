    ...
    TimeProvider clock)
{
    ...
    [Description(
        ...
        + "ONE whole session: one that clips every session it touches — nine hours of a trading day, say, "
        + "over a session that runs longer — is refused, naming the nearest whole session, rather than "
        + "answered with two empty lists that read as \"this instrument did not trade\". "
