    ...
    TimeProvider clock)
{
    ...
    [Description(
        ...
        + "read the edges of a window as inclusive of whole sessions only. The window MUST contain AT LEAST "
        + "ONE whole session: one inside which no session both opens and closes — nine hours of a trading "
        + "day, say, over a session that runs longer — is refused rather than answered with two empty lists "
        + "that read as \"this instrument did not trade\". The refusal names the NEAREST whole session's "
        + "bounds — nearest by how far the window would have to widen to reach it, scanning ACROSS a "
        + "weekend, a holiday or the maintenance break rather than stopping at one — so widening as told "
        + "always answers. Only a market closure longer than the scan leaves no session to name, and that "
        + "refusal carries no bounds and says only to widen. So a window holding ONLY non-trading days is "
        + "refused rather than reporting them as \"did not trade\"; that signal needs a window that also "
        + "holds at least one whole session. "
