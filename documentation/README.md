...
|---|---:|---|
| [`prd.md`](prd.md) | 14.0K | You need **what is required**, or you are citing an `R-#`. Ids are stable and never renumbered, and a citation that does not resolve here fails CI. |
| [`architecture.md`](architecture.md) | 21.9K | You need **how the pieces fit** — the cache-aside path, the projection, the transports. |
| [`mcp-tool-catalogue`](mcp-tool-catalog.md) | 23.4K | You are adding, changing or calling a tool. The tool surface is a contract; this is it. |
| [`data-dictionary.md`](data-dictionary.md) | 11.5K | You need the data model — the ten tables, their keys, and why each key is shaped that way. |
...
| [`deployment.md`](deployment.md) | 1.2K | You are operating a deployed environment — cost, what pages you (gh#526), WAF lockout (gh#528), and later rotation / restore / which release is running (gh#519, gh#523). |
