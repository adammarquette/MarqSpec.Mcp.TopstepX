...
|---|---:|---|
| [`prd.md`](prd.md) | 11.7K | You need **what is required**, or you are citing an `R-#`. Ids are stable and never renumbered, and a citation that does not resolve here fails CI. |
| [`architecture.md`](architecture.md) | 19.8K | You need **how the pieces fit** — the cache-aside path, the projection, the transports. |
...
| [`data-dictionary.md`](data-dictionary.md) | 9.7K | You need the data model — the ten tables, their keys, and why each key is shaped that way. |

## Working agreements

| Document | ~tok | Read it when |
|---|---:|---|
| [`AGENT-MEMORY.md`](AGENT-MEMORY.md) | 12.2K | **Before starting any work.** It grows by *append, don't overwrite* and shrinks by the retirement rule in its own header (gh#254), so this row moves in both directions — correct it in the pull request that moves the file. |
