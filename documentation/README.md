...
|---|---:|---|
| [`prd.md`](prd.md) | 15.1K | You need **what is required**, or you are citing an `R-#`. Ids are stable and never renumbered, and a citation that does not resolve here fails CI. |
| [`architecture.md`](architecture.md) | 23.8K | You need **how the pieces fit** — the cache-aside path, the projection, the transports. |
| [`mcp-tool-catalogue`](mcp-tool-catalog.md) | 24.9K | You are adding, changing or calling a tool. The tool surface is a contract; this is it. |
| [`data-dictionary.md`](data-dictionary.md) | 12.6K | You need the data model — the eleven tables, their keys, and why each key is shaped that way. |
## Working agreements

| Document | ~tok | Read it when |
|---|---:|---|
| [`AGENT-MEMORY.md`](AGENT-MEMORY.md) | 14.8K | **Before starting any work.** It grows by *append, don't overwrite* and shrinks by the retirement rule in its own header (gh#254), so this row moves in both directions — correct it in the pull request that moves the file. |
| [`project-board-workflow.md`](project-board-workflow.md) | 7.1K | You are filing, grooming or moving a card. **The board is project #5; #4 is retired.** The board makes two of the seven transitions by itself; the other five are somebody's deliberate act. |
| [`deployment.md`](deployment.md) | 13.5K | You are operating a deployed environment — account and region, staging zone lookup (`Z00545362JA49XMTT3U7Q`), pipeline deploy and rollback (gh#520), delete+redeploy after `ROLLBACK_COMPLETE`, Fargate quota 64, secret ARNs (gh#519), cost (gh#527), what pages you (gh#526), WAF lockout (gh#528), Observability (gh#537, gh#646), store restore (gh#522), postgres task stop and remaining-cash tape load (gh#525), which release / rotation / scale-to-zero / failed deploy (gh#523), MCP-host callback allowlist (gh#656). |
