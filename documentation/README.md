...
|---|---:|---|
| [`prd.md`](prd.md) | 7.9K | You need **what is required**, or you are citing an `R-#`. Ids are stable and never renumbered, and a citation that does not resolve here fails CI. |
| [`architecture.md`](architecture.md) | 9.5K | You need **how the pieces fit** — the cache-aside path, the projection, the transports. |
| [`mcp-tool-catalogue`](mcp-tool-catalog.md) | 14.4K | You are adding, changing or calling a tool. The tool surface is a contract; this is it. |
| [`data-dictionary.md`](data-dictionary.md) | 5.3K | You need the data model — the eight tables, their keys, and why each key is shaped that way. |
...
| [`../CONTRIBUTING.md`](../CONTRIBUTING.md) | 4.2K | Branching, claiming, commits, PRs, and the Definition of Done. |
