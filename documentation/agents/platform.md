...
- **Run a text-matching gate before believing its diagnostics.** Proving the above by mutation turned up a
  ...
  **And a self-test's needle is a text match too, so a needle that is a PREFIX of the right answer is
  ...
  the subject before believing the self-test: six of that file's eighteen cases exist because a mutation
...
- [`check-requirement-ids.sh`](../../scripts/check-requirement-ids.sh), which resolves **every `R-#` and
  ...
  six assertions on correct input it must accept. `check-doc-links.sh` proves a relative *link* resolves;
...
- **Neither rule reads fenced code, and that is not a leak** (PR #193 review). A document that *explains* a
  ...
  *this file*, each one `|` short of reddening its own build. A fence left **open** at end of file is the one
  ...
  time. **That strip then produced a false positive of its own, which is the shape to expect after a parser
  fix:** a fence opened inside a blockquote and closed by the **end of that blockquote** — valid CommonMark,
